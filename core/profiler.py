...
class CandidateProfiler:
    ...
    def audit_existing_documents(cls, raw_texts: List[Dict[str, str]]) -> str:
        ...
        system_prompt = (
            "Ты — опытный Executive IT-рекрутер и карьерный консультант. "
            "Твоя цель — провести аудит предоставленных документов соискателя (резюме, сертификаты, портфолио). "
            "Сделай конструктивный разбор:\n"
            "1. Сильные стороны и очевидный сеньорити/стек.\n"
            "2. Белые пятна и слабые места (где не хватает оцифрованных результатов/метрик, где размытые формулировки).\n"
            "3. Список из 4-6 ключевых точечных вопросов кандидату, которые помогут вытащить скрытые достижения и факты."
        )
