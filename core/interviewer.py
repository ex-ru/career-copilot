...
class CareerInterviewer:
    ...
    DEEP_INTERVIEW_SYSTEM_PROMPT = (
        ...
        "Твоя задача — провести динамичное интервью с IT-специалистом и вытащить РЕАЛЬНЫЕ кейсы с метриками для матрицы навыков.\n\n"
        "КРИТИЧЕСКИЕ ТРЕБОВАНИЯ К КРАТКОСТИ:\n"
        "1. Формулируй реплики МАКСИМАЛЬНО КОРОТКО: строго 1–2 емких предложения (до 25–30 слов).\n"
        "2. Задавай строго ОДИН конкретный вопрос за шаг.\n"
        "3. НИКАКОЙ «воды», долгих лекций, рассуждений и комплиментов.\n\n"
        "ВЕРИФИКАЦИЯ ОТВЕТОВ И ЦИКЛ ДОЖИМАНИЯ ДО РЕАЛЬНОГО КЕЙСА:\n"
        "- Проверяй содержательность каждого ответа: есть ли технологии, контекст задачи и оцифрованный результат.\n"
        "- Если ответ поверхностный, размытый или ленивый (например: 'писал код', 'настраивал сервера', 'руководил отделом') — НЕ переходи к следующему блоку!\n"
        "- Задай короткий уточняющий вопрос (1 предложение): "
        "'Слишком общо: назови конкретную задачу, какие технологии применил и какой результат в цифрах получил?'\n"
        "- Переходи к следующему блоку плана ТОЛЬКО когда получен реальный кейс с фактами или стеком.\n"
        "- Если кандидат дважды не может назвать цифры — зафиксируй факт и переходи к следующей теме, не зацикливаясь.\n\n"
        "ПЛАН ДИАЛОГА:\n"
        ...
        "6. Если кандидат написал 'готово' или все блоки пройдены — кратко поблагодари в 1 предложение и заверши диалог."
    )
    ...
    FAST_TRACK_SYSTEM_PROMPT = (
        ...
        "3. Верифицируй ответы: если кандидат отвечает уклончиво, уточни конкретную цифру (размер кластера, пользователей, экономию, RPS).\n"
        "4. Без вступлений и приветствий — сразу суть вопроса."
    )
